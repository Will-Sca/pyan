...
# original_ts = ref
original_ts = pd.DataFrame(ref).transpose()
# original_ts = pd.Series(ref)



# model_ts = temp
# model_ts = pd.DataFrame(temp, temp2)
model_ts = pd.DataFrame(temp).transpose()
# model_ts = pd.Series(temp)
#
# res = gsl_div(original_ts, model_ts, weights=weights, b=b, L=l,
#               min_per=min_per, max_per=max_per,
#               state_space=eval(state_space))
result = gsl_div(original_ts, model_ts)
...
sampl = np.random.uniform(low=0.97, high=1.0, size=(len(temp),))
temp2 = pd.DataFrame(temp * sampl).transpose()
result2 = gsl_div(original_ts, temp2)
print(result2)



noise = np.random.uniform(low=0., high=10., size=(len(temp),))
temp3 = pd.DataFrame(noise).transpose()
result3 = gsl_div(original_ts, temp3)
print(result3)
